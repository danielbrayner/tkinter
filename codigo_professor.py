...
def cadastrar():
    ...
    matricula = int(inMatricula.get())
    dt_nascimento = str(indt_nascimento.get()).strip().title()

    cur.execute(f"INSERT INTO alunos(nome, matricula, dt_nascimento) VALUES ('{nome}', {matricula}, '{dt_nascimento}')")
...
indt_nascimento = Entry(cadastro_frame)

inNome.grid(row=3, column=1)
inMatricula.grid(row=4, column=1)
indt_nascimento.grid(row=5, column=1)
